...
void EudmManager::ConstructPlainOutput(planning::eudm::PlainOutput* out) {
  ...
  // * pack elements
  {
    for (int i = 0; i < num_seqs; i++) {
      ...
      if (!last_snapshot_.sim_res[i]) {
        ...
        // consistency cost
        ele.final_cost = last_snapshot_.final_cost[i];
        // * macro actions
        ...
        for (auto& cost : last_snapshot_.progress_cost[i]) {
        ...
        auto tail_cost = last_snapshot_.tail_cost[i];
        ele.tail_cost =
            PlainCost(tail_cost.valid_sample_index_ub,
                      tail_cost.efficiency.ave(), tail_cost.safety.ave(),
                      tail_cost.navigation.ave(), tail_cost.weight);
        // * micro states
  ...
  // * ego state
  {
    auto state = last_snapshot_.plan_state;
    out->ego_state = PlainState(
        state.time_stamp, state.vec_position[0], state.vec_position[1],
        state.angle, state.velocity, state.acceleration, state.curvature);
  }
  out->winner_element_id = last_snapshot_.processed_winner_id;
  out->original_winner_id = last_snapshot_.original_winner_id;
  out->best_cost =
      last_snapshot_.final_cost[last_snapshot_.processed_winner_id];
  out->ongoing_action_duration = last_snapshot_.action_script.front().front().t;
  out->plan_stamp = last_snapshot_.plan_stamp;
  out->time_cost = last_snapshot_.time_cost;
}
